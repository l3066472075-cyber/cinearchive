...
def respond_to_note(
    ...
    focus = (
        "请基于他写的这份观影笔记，给他一段**整体而深入的回应**（约 300~400 字，分 3~4 个自然段，用换行分隔），"
        "像一位懂电影也懂人心、深谙「观电影法」的老朋友。层层递进：\n"
        "① 先看见并接住：他最触动的那个片段、那句台词，背后照见了他的什么——扣住他写下的具体内容，不要泛泛；\n"
        "② 借电影观自己：带他「退回到观者」重新看这一幕——这段情节、这个角色，像不像他人生剧本里的某一幕？"
        "尤其要郑重对待他写下的「像你人生脚本的地方」这一条，那是他自己照见自己的地方；\n"
        "③ 温和的觉察：他记下的触动与思考之间，是否有一条反复出现的线（模式）？用开放、邀请式的语气轻轻点一点，不武断、不下结论、不诊断；\n"
        "④ 收束：把这份觉察轻轻收好，落在一句温暖有力的看见或鼓励上——借电影观自己，跳出人生这场戏，带着觉知勇敢入戏。"
        "不要说教、不贴标签、不喊口号。"
    )
...
{_HUMAN_TOUCH}"""
    return lc.llm_generate(_ROLE_SYSTEM[role], prompt, max_tokens=1000)
